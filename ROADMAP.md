...
- [ ] **Phase G.7 — Tier 2 adapter batch (broaden what jobs can read and act on)** — Gmail is the only email source today; that is a limitation of what is connected, not of the design. In priority order, each behind the same adapter seam, each with a real "Send test" round-trip and typed failure codes, none shipping without one:
  - [ ] **Microsoft 365 / Outlook** (OAuth via the Microsoft app-user connector) — the default mailbox for SMBs outside China.
  - [ ] **IMAP/SMTP generic mail** — covers every other mailbox with no vendor approval; read + draft, never send without approval.
  - [ ] **WeChat Work (企业微信)** — customer-contact and group messaging for mainland businesses.
  - [ ] **Feishu / DingTalk** — notifications and approvals for mainland teams.
  - [ ] **Stripe / invoicing reads** — overdue invoices arrive automatically instead of being typed in (feeds the existing invoice-chase job).
  - [ ] **Google Sheets** — read business facts (price lists, stock, bookings) for the cited Q&A job.
  - [ ] **Outbound webhooks v2** — managed retry queue and per-endpoint signing secrets, so anything without a native adapter can still be reached.
